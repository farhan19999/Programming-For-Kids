...
const cCodeRunner = async (
  ...
  for (let testcase of testcases) {
    ...
    const result2 = await load_test_cases(
      testcase.input_file,
    ...
    try {
      ...
      const { pid, stdout, stderr } = await exec(
        `"./app/files/a.exe" < ./app/files/${testcase.input_file}`,
        { timeout: time_limit }
      );
      const data = await fs.readFile(
        `./app/files/${testcase.output_file}`,
        "utf8"
      );
      if (data === stdout) {
        ...
        console.log(
          `testcase: ${testcase.input_file} -> Expected: ${data} \t Got: ${stdout}`
        );
        verdict = "WA";
      }
